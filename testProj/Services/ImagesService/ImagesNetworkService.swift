...
protocol ImagesNetworkServiceProtocol: AnyObject {
    
    func fetchImages(page: Int) async throws -> [Photo]
}

final class ImagesNetworkService: ImagesNetworkServiceProtocol {
    ...
    func fetchImages(page: Int) async throws -> [Photo] {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            .responseDecodable(of: ImagesResponseModel.self) { response in
                
                switch response.result {
                    
                case .success(let success):
                    
                    let photos = success.photos?.photos ?? []
